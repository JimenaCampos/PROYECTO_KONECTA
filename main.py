    ...
    if request.method == 'POST':
        ...
        if not usuario_form or not password_form:
            ...
            if resultado_usuario:
                if resultado_usuario.get('primer_ingreso') in [1, True]:
...
def dashboard(usuario_id):
    ...
    incidencias_datos = listarIncidencias(solo_propios=es_colaborador, usuario_id=usuario_id, pagina=1, por_pagina=8)
    resumen_datos = reporteResumen() if es_tecnico_o_jefe else {}
    stock_datos = stockPCs() if es_tecnico_o_jefe else {}
    
    return render_template('dashboard.html', id_del_usuario=usuario_id, user=user, incidencias=incidencias_datos, resumen=resumen_datos, stock=stock_datos)
...
def mi_perfil(usuario_id):
    ...
    if request.method == 'POST':
        ...
        if accion == 'datos':
            ...
            elif cel and (not cel.isdigit() or len(cel) != 9): 
                error = 'El celular debe tener 9 dígitos'
            else:
...
def cambiar_password(usuario_id):
    ...
    if request.method == 'POST':
        ...
        if not all([pa, pn, pc]): 
            ...
            error = msg
    return render_template('form_cambiar_password.html', id_del_usuario=usuario_id, user=user, error=error)
...
def usuarios(usuario_id):
    ...
    if q:     
        todos = [u for u in todos if q.lower() in u['nombre'].lower() or q.lower() in u['usuario'].lower()]
    if rol_f: 
        todos = [u for u in todos if u['rol_nombre'] == rol_f]
        
    total  = len(todos)
    tp     = max(1, -(-total // pp))
    pagina = max(1, min(pagina, tp))
    lista  = todos[(pagina-1)*pp : pagina*pp]
    roles_datos = obtenerRoles()
    user_actual = obtenerUsuario(usuario_id)
    
    return render_template('lista_usuarios.html', id_del_usuario=usuario_id, user=user_actual, usuarios=lista, roles=roles_datos, total=total, pagina=pagina, total_pags=tp, q=q, rol_f=rol_f, error=None, mensaje=None)
...
def nuevo_usuario(usuario_id):
    ...
    return render_template('lista_usuarios.html', id_del_usuario=usuario_id, user=user_actual, usuarios=todos[:10], roles=roles_datos, total=len(todos), pagina=1, total_pags=max(1, -(-len(todos)//10)), q='', rol_f='', error=error, mensaje='Usuario creado exitosamente' if not error else None)
...
def editar_usuario(usuario_id, id_usuario_destino):
    ...
    roles_datos = obtenerRoles()
    user_actual = obtenerUsuario(usuario_id)
    return render_template('form_usuario_edit.html', id_del_usuario=usuario_id, user=user, roles=roles_datos, error=error)
...
def estado_usuario(usuario_id, id_usuario_destino, activo):
    if id_usuario_destino != usuario_id: 
        cambiarEstadoUsuario(id_usuario_destino, activo)
    return redirect(url_for('usuarios', usuario_id=usuario_id))


# ─── GESTIÓN DE INCIDENCIAS (SINTAXIS DIRECTA ESTILO PELÍCULAS) ───
...
def lista_incidencias(usuario_id):
    ...
    # Captura directa de datos desde la clase AD (Igual a datos = leerPeliculas())
    resultado_incidencias = listarIncidencias(solo_propios=es_colaborador, usuario_id=usuario_id, pagina=1, por_pagina=100, orden=orden)
    
    # Envío limpio de parámetros al HTML
    return render_template('lista_incidenciaPrincipal.html', id_del_usuario=usuario_id, user=user, incidencias=resultado_incidencias)


@app.route("/incidencias/<int:usuario_id>/nueva")
def cargar_formulario_incidencia(usuario_id):
    user = obtenerUsuario(usuario_id)
    tipos_datos = obtenerTiposIncidencia()
    return render_template('form_incidencia.html', id_del_usuario=usuario_id, user=user, tipos=tipos_datos)
...
def ver_detalle_incidencia(usuario_id, idIncidencia):
    user = obtenerUsuario(usuario_id)
    incidencia_data = obtenerIncidenciaxId(idIncidencia)
    tecnicos_data = obtenerTecnicos()
    return render_template('form_inicidencia_edit.html', id_del_usuario=usuario_id, user=user, incidencia=incidencia_data, tecnicos=tecnicos_data, rol_del_usuario=user['rol_nombre'])
...
def actualizar_estado_incidencia(usuario_id):
    id_incidencia = request.form['idIncidencia']
    if actualizarEstadoIncidencia(id_incidencia, request.form['estado'], request.form['diagnostico'].strip(), request.form['solucion'].strip()):
        return redirect(url_for('ver_detalle_incidencia', usuario_id=usuario_id, idIncidencia=id_incidencia))
...
def dashboard_incidencias_estrategico(usuario_id):
    ...
    resumen_data = reporteResumen()
    por_tipo_data = reportePorTipo()
    por_tecnico_data = reportePorTecnico()
    proyeccion_data = calcularProyeccionMensual()
    incidencias_data = listarIncidencias(solo_propios=es_colaborador, usuario_id=usuario_id, pagina=1, por_pagina=10)
    
    return render_template('form_dashboard_Incidencias.html', id_del_usuario=usuario_id, user=user, resumen=resumen_data, por_tipo=por_tipo_data, por_tecnico=por_tecnico_data, proyeccion=proyeccion_data, incidencias=incidencias_data)


# ─── 🛡️ NUEVA RUTA: COMPROBANTES (EVITA ERRORES DE REDIRECCIÓN EN EL NAVBAR) ───
@app.route("/comprobantes/<int:usuario_id>")
def comprobantes(usuario_id):
    user = obtenerUsuario(usuario_id)
    # Vista simple de contingencia para que el sistema no explote si hacen clic en el menú
    return f"<h3>Módulo de Comprobantes de Pago - Usuario: {user['nombre']}</h3><p>Próximamente disponible.</p><br><a href='/incidencias/{usuario_id}'>Volver al inicio</a>"
...
def inventario(usuario_id):
    ...
    todas    = obtenerPCs()
    if q:        
        todas = [p for p in todas if q.lower() in p['hostname'].lower() or q.lower() in (p['marca_modelo'] or '').lower() or q.lower() in (p['numero_serie'] or '').lower()]
    if estado_f: 
        todas = [p for p in todas if p['estado'] == estado_f]
    if area_f:   
        todas = [p for p in todas if str(p.get('area_id') or '') == area_f]
        
    total  = len(todas)
    tp     = max(1, -(-total // pp))
    pagina = max(1, min(pagina, tp))
    pcs_datos = todas[(pagina-1)*pp : pagina*pp]
    stock_datos = stockPCs()
    tecnicos_datos = obtenerTecnicos()
    areas_datos = listarAreas()
    user_actual = obtenerUsuario(usuario_id)
    
    return render_template('lista_inventario.html', id_del_usuario=usuario_id, user=user_actual, pcs=pcs_datos, stock=stock_datos, tecnicos=tecnicos_datos, areas=areas_datos, total=total, pagina=pagina, total_pags=tp, q=q, estado_f=estado_f, area_f=area_f)
...
def nueva_pc(usuario_id):
    ...
    areas_datos = listarAreas()
    user_actual = obtenerUsuario(usuario_id)
    return render_template('lista_inventario.html', id_del_usuario=usuario_id, user=user_actual, pcs=todas[:10], stock=stock_datos, tecnicos=tecnicos_datos, areas=areas_datos, total=len(todas), pagina=1, total_pags=max(1, -(-len(todas)//10)), q='', estado_f='', area_f='', error=error)
...
def editar_pc(usuario_id, pc_id):
    ...
    user_actual = obtenerUsuario(usuario_id)
    pc_data = obtenerPC(pc_id)
    tecnicos_datos = obtenerTecnicos()
    areas_datos = listarAreas()
    return render_template('form_pc_edit.html', id_del_usuario=usuario_id, user=user_actual, pc=pc_data, error=error, tecnicos=tecnicos_datos, areas=areas_datos)
...
def componentes(usuario_id):
    ...
    if q:      
        todos = [c for c in todos if q.lower() in c['nombre'].lower()]
    if tipo_f: 
        todos = [c for c in todos if c['tipo'] == tipo_f]
    ...
    return render_template('lista_componentes.html', id_del_usuario=usuario_id, user=user_actual, componentes=lista, total=total, pagina=pagina, total_pags=tp, q=q, tipo_f=tipo_f)
...
def nuevo_componente(usuario_id):
    ...
    todos = obtenerComponentes()
    user_actual = obtenerUsuario(usuario_id)
    return render_template('lista_componentes.html', id_del_usuario=usuario_id, user=user_actual, componentes=todos[:10], total=len(todos), pagina=1, total_pags=max(1, -(-len(todos)//10)), q='', tipo_f='', error=error)
...
def editar_componente(usuario_id, comp_id):
    ...
    user_actual = obtenerUsuario(usuario_id)
    comp_data = obtenerComponente(comp_id)
    return render_template('form_componente_edit.html', id_del_usuario=usuario_id, user=user_actual, comp=comp_data, error=error)
...
def estructura(usuario_id):
    user_actual = obtenerUsuario(usuario_id)
    areas_datos = listarAreas()
    return render_template('lista_estructura.html', id_del_usuario=usuario_id, user=user_actual, areas=areas_datos)
...
def estructura_areas(usuario_id):
    user_actual = obtenerUsuario(usuario_id)
    areas_datos = listarAreas()
    return render_template('lista_areas.html', id_del_usuario=usuario_id, user=user_actual, areas=areas_datos, error=None, mensaje=None)
...
def nueva_area(usuario_id):
    ...
    if not n: 
        ...
        if insertarArea(Area(n, des, res)): 
    ...
    user_actual = obtenerUsuario(usuario_id)
    areas_datos = listarAreas()
    return render_template('lista_areas.html', id_del_usuario=usuario_id, user=user_actual, areas=areas_datos, error=error, mensaje=mensaje)
...
def editar_area(usuario_id, area_id):
    ...
    user_actual = obtenerUsuario(usuario_id)
    return render_template('form_area_edit.html', id_del_usuario=usuario_id, user=user_actual, area=area, error=error)
...
def estructura_usuarios_area(usuario_id):
    user_actual = obtenerUsuario(usuario_id)
    usuarios_datos = listarUsuariosConArea()
    areas_datos = listarAreas()
    return render_template('lista_usuarios_area.html', id_del_usuario=usuario_id, user=user_actual, usuarios=usuarios_datos, areas=areas_datos, error=None, mensaje=None)
...
def asignar_area_usuario(usuario_id):
    ...
    if not uid or not aid: 
        error = 'Selecciona un usuario y un área de la lista'
    else:
        if asignarAreaAUsuario(int(uid), int(aid)): 
    ...
    usuarios_datos = listarUsuariosConArea()
    areas_datos = listarAreas()
    return render_template('lista_usuarios_area.html', id_del_usuario=usuario_id, user=user_actual, usuarios=usuarios_datos, areas=areas_datos, error=error, mensaje=mensaje)
...
def editar_usuario_area(usuario_id, id_usuario_destino):
    ...
    areas_datos = listarAreas()
    return render_template('form_usuario_area_edit.html', id_del_usuario=usuario_id, user=user, areas=areas_datos, error=error)


@app.route('/estructura/<int:usuario_id>/pcs_usuario')
def estructura_pcs_usuario(usuario_id):
    user_actual = obtenerUsuario(usuario_id)
    pcs_datos = listarPCsConUsuario()
    usuarios_datos = listarUsuariosSinPC()
    return render_template('lista_pcs_usuario.html', id_del_usuario=usuario_id, user=user_actual, pcs=pcs_datos, usuarios=usuarios_datos, error=None, mensaje=None)
...
def asignar_usuario_pc(usuario_id):
    ...
    if not pid: 
        error = 'Selecciona una PC'
    else:
        if asignarUsuarioAPC(int(pid), int(uid) if uid else None): 
            mensaje = 'Asignación de equipo guardada'
        else: 
            error = 'Error al guardar la asignación'
    ...
    pcs_datos = listarPCsConUsuario()
    usuarios_datos = listarUsuariosSinPC()
    return render_template('lista_pcs_usuario.html', id_del_usuario=usuario_id, user=user_actual, pcs=pcs_datos, usuarios=usuarios_datos, error=error, mensaje=mensaje)
...
def editar_pc_usuario(usuario_id, pc_id):
    ...
    user_actual = obtenerUsuario(usuario_id)
    usuarios_datos = listarUsuariosSinPC()
    return render_template('form_pc_usuario_edit.html', id_del_usuario=usuario_id, user=user_actual, pc=pc, usuarios=usuarios_datos, error=error)
